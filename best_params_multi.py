...
from export import export_parameter_comparison_latex, generate_subfigure
...
if __name__ == '__main__':
    ...
    point_count = 10  # Point count to test for
    ...
    for i, (theta_name, _main_methods) in enumerate(test_params.items()):
        ...
        for j, (main_method_name, _ls_methods) in \
            ...
            for k, (line_search_name, test) in enumerate(_ls_methods.items()):
                _test_params = test['params']
                _test_ls_params = test['ls_params']

                # Create the permutations of parameters
                permutations = list(itertools.product(
                    *[*_test_params.values(), *_test_ls_params.values()]
                ))
                task_count += len(permutations) * len(x0s)
                param_keys = [*_test_params.keys()]
                ls_param_keys = [*_test_ls_params.keys()]
                ...
                for p in permutations:
                    ...
                    for i, x0 in enumerate(x0s):
                        args = {
                            ...
                            'test_ls_params': _test_ls_params,
    ...
    for th in result_dict.values():
        for mm in th.values():
            for ls in mm.values():
                print(f'==> BEST: {str(min(ls))}')

    print(f'\nProgram completed in {time.time() - start_time} seconds.')

    # print(generate_subfigure(result_dict, 'MatrixSquareSum', 'NewtonsMethod', 'DichotomousSearch'))

    # export_parameter_comparison_latex(result_dict, point_count)
